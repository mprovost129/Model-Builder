  ...
  return (
    <div className="story-manager-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
      <section className="story-manager product-library-manager" role="dialog" aria-modal="true" aria-labelledby="product-library-title">
            ...
            {entries.map((entry) => {
              ...
              return <article className={entry.isActive ? "product-library-card is-active" : "product-library-card"} key={entry.id}>
                ...
                {storedSvg ? <div className="product-library-asset-preview">
                  {/* eslint-disable-next-line @next/next/no-img-element -- Private uploaded SVGs are served through the guarded asset endpoint, not the public image optimizer. */}
        ...
        {assetImport ? <div className="product-asset-import-backdrop" role="presentation">
          <section className="product-asset-import" role="dialog" aria-modal="true" aria-labelledby="product-asset-import-title">
            <header><div><strong id="product-asset-import-title">Add Product Representation</strong><span>{building.openingTypes.find((type) => type.id === assetTargetId)?.name} · {assetImport.file.name}</span></div><button type="button" onClick={() => { setAssetImport(null); setAssetImportError(""); }} aria-label="Close representation import">×</button></header>
            <div className="product-asset-import-body">
              <dl><div><dt>Format</dt><dd>{assetImport.format.toUpperCase()}</dd></div><div><dt>File size</dt><dd>{(assetImport.file.size / 1024).toLocaleString(undefined, { maximumFractionDigits: 1 })} KB</dd></div></dl>
              <label><span>Representation name</span><input value={assetImport.name} maxLength={100} onChange={(event) => setAssetImport((current) => current ? { ...current, name: event.target.value } : current)} /></label>
              ...
              <p>SVG files are checked for executable and externally loaded content. GLB files must use the version-2 binary format. Accepted bytes are stored privately; the project saves only the asset manifest and checksum.</p>
